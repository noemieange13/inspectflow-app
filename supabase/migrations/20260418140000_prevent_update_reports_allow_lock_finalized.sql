-- Évite que le trigger « immutable » bloque les déverrouillages qui ne touchent qu’à
-- is_locked / finalized_at (ex. unlockReportRowForEdit dans lib/updateReportPayloadWithUnlock.ts)
-- lorsque generating / generating_at sont déjà à false / NULL.

alter table public.reports add column if not exists is_locked boolean;
alter table public.reports add column if not exists finalized_at timestamptz;

create or replace function public.prevent_update_reports()
returns trigger
language plpgsql
security definer
set search_path = public
as $$
begin
  if (
    OLD.payload is distinct from NEW.payload
    or OLD.pdf_path is distinct from NEW.pdf_path
    or OLD.pdf_url is distinct from NEW.pdf_url
    or OLD.generating is distinct from NEW.generating
    or OLD.generating_at is distinct from NEW.generating_at
    or OLD.status is distinct from NEW.status
    or OLD.access_token is distinct from NEW.access_token
    or OLD.token_expires_at is distinct from NEW.token_expires_at
    or OLD.first_view_notified is distinct from NEW.first_view_notified
    or OLD.is_locked is distinct from NEW.is_locked
    or OLD.finalized_at is distinct from NEW.finalized_at
  ) and (
    OLD.id = NEW.id
    and OLD.user_id is not distinct from NEW.user_id
    and OLD.report_id is not distinct from NEW.report_id
    and OLD.inspection_id is not distinct from NEW.inspection_id
  ) then
    return NEW;
  end if;

  raise exception 'Report is immutable'
    using errcode = 'P0001';
end;
$$;

comment on function public.prevent_update_reports() is
  'Autorise les mises à jour pipeline (payload, pdf_path, verrous generating, statut, jetons, verrou édition is_locked/finalized_at) si les clés métier id/user_id/report_id/inspection_id sont inchangées.';
