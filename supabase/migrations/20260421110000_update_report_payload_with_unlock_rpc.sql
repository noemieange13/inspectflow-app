-- Mise à jour atomique du payload (déverrouillage optionnel + invalidation PDF) en une transaction,
-- avec verrouillage de ligne (FOR UPDATE) pour sérialiser les écritures concurrentes.
...
comment on function public.update_report_payload_with_unlock(uuid, jsonb, text, boolean, boolean) is
  'Service role : met à jour payload (et optionnellement pdf_path) ; déverrouille si is_locked et p_allow_unlock. Sérialise par FOR UPDATE sur la ligne.';
