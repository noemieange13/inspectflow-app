...
type ReportViewerAccessResult =
  | { ok: true }
  | { ok: false; status: number; body: Record<string, unknown> };

export function evaluateReportViewerAccess(
  record: Record<string, unknown>,
  accessTokenRaw: string | null | undefined,
): ReportViewerAccessResult {
  const dbToken = typeof record.access_token === "string" ? record.access_token.trim() : "";
...
export async function assertReportViewerAccess(
  ...
  return evaluateReportViewerAccess(report as Record<string, unknown>, accessTokenRaw);
}
