...
export async function loadReportForViewer(
  ...
  try {
    ...
    const accessGate = validateReportAccessRow(reportId, viewerToken ?? "", rec);
    if (!accessGate.ok) {
      return {
        id: reportId,
        status: null,
        ...
        hasPdf: false,
        pdfSignedUrl: null,
        accessDenied: accessGate.status === 403,
        notFound: accessGate.status === 404,
        serverError: accessGate.status >= 500 ? accessGate.error : undefined,
      };
    }
