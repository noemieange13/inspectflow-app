...
export default function InspectionCoverForm({
  ...
  const onLogo = useCallback((file: File | null) => {
    ...
    reader.onload = () => {
      ...
      setIaMessage(
        linkedToReport
          ? "Logo mis à jour — enregistrez sur le rapport pour l’inclure au PDF."
          : "Logo enregistré localement (aperçu page couverture — export PDF à brancher).",
      );
    };
    reader.readAsDataURL(file);
  }, [data.inspecteur_nom, data.inspecteur_numero_certification, data.compagnie, linkedToReport]);

  const stubIa = useCallback(async (label: string) => {
    setIaMessage(null);
    try {
      ...
      setIaMessage(
        `${label} — ${j.message ?? "Phase 2 : brancher vision / OCR sur Edge."}`,
      );
    } catch {
      setIaMessage(
    ...
    <div className="space-y-10">
      ...
      >
        <p className="font-medium">
          {linkedToReport
            ? `Formulaire lié au rapport ${reportId?.slice(0, 8)}…`
            : "Prototype formulaire couverture"}
        </p>
        <p className={`mt-1 ${linkedToReport ? "text-emerald-900/95" : "text-amber-900/90"}`}>
          {linkedToReport
            ? "Les données peuvent être enregistrées dans le rapport (payload Supabase) et incluses au PDF lors de la prochaine génération. Les boutons « IA » restent des stubs explicites."
            : "Tous les champs visibles sur tes captures Word sont présents ci-dessous. Les fonctions « IA » sont câblées en bouton explicite : la prochaine étape est de les relier à des Edge Functions (vision / OCR / audio) sans retirer la saisie manuelle."}
        </p>
        {linkedToReport ? (
          <p className="mt-2 flex flex-wrap items-center gap-x-3 gap-y-2">
            <Link
              ...
              className="inline-flex rounded-md bg-emerald-800 px-3 py-1.5 text-sm font-semibold text-white shadow-sm hover:bg-emerald-900"
            >
              Ouvrir le rapport — constats et PDF
            </Link>
            <span className="text-xs text-emerald-900/85">
              Les constats (« Ajouter un constat ») et la génération PDF sont sur cette page, pas ici.
            </span>
          </p>
        ) : null}
      </div>
        ...
        <div className="flex flex-wrap gap-2">
          <label className="inline-flex cursor-pointer items-center gap-2 rounded-md border border-dashed border-slate-300 px-3 py-2 text-sm">
            <input
              ...
              accept="image/*"
              className="hidden"
              onChange={() => void stubIa("Analyse photo DV")}
            />
        ...
        {data.description_sommaire.mode === "photos_ia" ? (
          <div>
            <input
              ...
              multiple
              className="text-sm"
              onChange={() => void stubIa("Analyse photos description sommaire")}
            />
          </div>
        ) : null}
        <div className="grid gap-4 md:grid-cols-2">
      ...
      <section className="space-y-4">
        ...
        <button
          ...
          onClick={() => void stubIa("Synthèse condition générale depuis lot de photos")}
        >
      ...
      <section className="space-y-4">
        ...
        <button
          type="button"
          className="rounded-md border border-slate-300 bg-white px-3 py-1.5 text-xs font-medium"
          onClick={() => void stubIa("Orientation façade (vision / plan)")}
        >
