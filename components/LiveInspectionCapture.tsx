...
type Props = {
  ...
  onPhotoUploaded?: (result: { photo_id: string | null; batch_id?: string | null }) => void;
};

/**
 * Capture depuis la caméra → outbox IndexedDB → `/api/upload-photo`.
 */
export default function LiveInspectionCapture({
  ...
  }, [reportId, language, onPhotoUploaded, sequenceNumber]);
    ...
    <div className="mt-3 rounded-lg border border-slate-200 bg-slate-50/80 px-3 py-2">
      ...
      {open ? (
        <div className="mt-2 space-y-2">
          <video ref={videoRef} className="max-h-48 w-full rounded-md bg-black object-contain" playsInline muted />
          <button
            type="button"
