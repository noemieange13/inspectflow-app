...
export async function POST(req: Request) {
  ...
  const weatherRaw =
    typeof body === "object" && body !== null && INSPECTION_WEATHER_PAYLOAD_KEY in body
      ? (body as Record<string, unknown>)[INSPECTION_WEATHER_PAYLOAD_KEY]
      : undefined;
  ...
  const parsedWeather = parseInspectionWeatherV1(weatherRaw);
  if (!parsedWeather) {
    return Response.json(
      { success: false, error: "Invalid or missing inspection_weather_v1" },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    const nextPayloadRaw: Record<string, unknown> = {
      ...currentPayload,
      [INSPECTION_WEATHER_PAYLOAD_KEY]: parsedWeather,
    };

    const nextPayload = appendAuditTrail(nextPayloadRaw, {
      field_path: INSPECTION_WEATHER_PAYLOAD_KEY,
      old_preview: "[previous weather]",
      new_preview: `${parsedWeather.temperature_c}°C ${parsedWeather.condition}`,
      source: "inspection_weather_save",
    });
    ...
    return Response.json({
      success: true,
      report_id: reportId,
      inspection_weather_v1: parsedWeather,
    });
  } catch (error) {
