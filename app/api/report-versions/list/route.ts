...
export async function POST(req: Request) {
  ...
  try {
    ...
    // 🧠 Init Supabase après validation de l'entrée
    ...
    // 🔒 Gate admin legacy : les rapports sans jeton DB restent accessibles seulement à l'admin.
    if (!dbToken) {
      ...
      if (!gate.ok) {
        return Response.json(
          { data: [], error: gate.error, meta: { max_versions: MAX_VERSIONS } },
          { status: gate.status }
        )
      }
    }
