...
import { rejectMissingExactTriggerSecret } from "@/lib/triggerSecretAuth";

/**
 * Crée une ligne `reports` via l’Edge `create-report` (job_id résolu depuis l’inspection si omis).
 * Si `TRIGGER_INSPECTION_SECRET` est défini, seul `x-trigger-secret` exact est accepté.
 */
export async function POST(req: Request) {
  const authError = rejectMissingExactTriggerSecret(req);
