...
import {
  isTriggerSecretAuthorized,
  triggerSecretUnauthorizedResponse,
} from "@/lib/triggerSecretAuth";

/**
 * Crée une ligne `reports` via l’Edge `create-report` (job_id résolu depuis l’inspection si omis).
 * Garde d’auth optionnelle par secret lorsque `TRIGGER_INSPECTION_SECRET` est défini.
 */
export async function POST(req: Request) {
  if (!isTriggerSecretAuthorized(req)) {
    return triggerSecretUnauthorizedResponse();
  }
