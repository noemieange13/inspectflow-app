...
import { assertExactTriggerSecret } from "@/lib/triggerSecretAuth";

/**
 * Crée une ligne `reports` via l’Edge `create-report` (job_id résolu depuis l’inspection si omis).
 * Réservé aux appels serveur/scripts : exige `x-trigger-secret`.
 */
export async function POST(req: Request) {
  const secretGate = assertExactTriggerSecret(req);
  if (!secretGate.ok) {
