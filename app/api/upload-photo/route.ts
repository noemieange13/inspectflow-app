...
export async function POST(req: Request) {
  try {
    ...
    const gate = checkReportAccessToken(rec, accessTokenRaw);
    if (!gate.ok) {
      return Response.json(
        { error: gate.error, code: "access_denied" },
        { status: 403 },
      );
    }
    ...
    if (!effectiveInspectionId) {
      return Response.json(
        {
          error:
            "Report is missing inspection_id; cannot attach photo to inspection",
        },
        { status: 400 },
      );
    }
    ...
    if (!insertRes.error && insertRes.data?.id) {
      ...
      if (!photoId) {
        return Response.json(
          {
            error: "Photo upload could not be attached to inspection",
            details: insertRes.error?.message ?? null,
          },
          { status: 500 },
        );
      }
