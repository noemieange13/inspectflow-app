...
export async function POST(req: Request) {
  try {
    ...
    const gate = validateReportAccessRow(reportId.trim(), accessTokenRaw, report);
    if (!gate.ok) {
      return Response.json(
    ...
    if (!effectiveInspectionId) {
      return Response.json(
        { error: "Missing inspection_id for report photo upload" },
        { status: 400 },
      );
    }
        ...
        return Response.json(
          {
            ...
            details: insertRes.error?.message ?? "photos row was not created",
          },
          { status: 500 },
        );
      }
