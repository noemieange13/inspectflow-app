  ...
  try {
    ...
    const reportInspectionId =
      typeof report.inspection_id === "string" && report.inspection_id.trim()
        ...
        : null;
    const requestInspectionId = inspectionId?.trim() || null;
    if (
      reportInspectionId &&
      requestInspectionId &&
      requestInspectionId !== reportInspectionId
    ) {
      return Response.json(
        { error: "inspection_id does not match report" },
        { status: 400 },
      );
    }
    const effectiveInspectionId = reportInspectionId || requestInspectionId;
    const ownerId =
      typeof report.user_id === "string" ? report.user_id : "anonymous";

    if (!effectiveInspectionId) {
      return Response.json(
        { error: "Missing inspection_id for photo metadata" },
        { status: 400 },
      );
    }
    ...
    if (!insertRes.error && insertRes.data?.id) {
      ...
      if (dup) {
        ...
        if (existing?.id) {
          ...
          return Response.json(
            {
              error: "Photo metadata insert conflicted",
              details: existingErr?.message ?? insertRes.error?.message ?? null,
            },
            { status: 409 },
          );
        }
        ...
        return Response.json(
          { error: "Photo metadata insert failed", details: insertRes.error?.message ?? null },
          { status: 500 },
        );
      }
    }
