...
export async function POST(req: Request) {
  ...
  try {
    if (!hasExactTriggerSecret(req)) {
      ...
      if (reportError) {
        return Response.json({ success: false, error: reportError.message }, { status: 500 });
      }

      const gate = await assertReportAccessWithOptionalSession(req, report_id, accessTokenRaw, report);
      if (!gate.ok) {
        return Response.json(
